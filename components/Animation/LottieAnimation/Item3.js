...
export const RenderItem = ({cb, item, index, loopArray}) => {
  ...
  const unLockedItemAnimation = () => {
    grayToColorAnimation.value = withTiming(
      ...
      () => {
        lottieOpacityAnimation.value = withTiming(0.8, {
          duration: 100,
          easing: Easing.linear,
          useNativeDriver: false,
        });
  ...
  const opacityCal = () => {
    console.log('animatedStyles3', animatedStyles3);
    return nowPlay == item.id ? animatedStyles2 : animatedStyles3;
  };
  let lottieStyle = {
    transform: [
  ...
  useEffect(() => {
    ...
    // }
  }, [nowPlay, setNowPlay, loopArray, lottieOpacityAnimation.value]);
      ...
      <TouchableOpacity style={[styles.imageView]} onPress={() => onPress()}>
        ...
        <View style={{height: '100%', width: '100%'}}>
          ...
          {nowPlay === item.id && (
            <>
              <Animated.Image
                style={[styles.albumImage, opacityCal]}
                source={e43_Good}
              />
